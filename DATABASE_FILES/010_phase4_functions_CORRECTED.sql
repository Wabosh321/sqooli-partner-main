...
-- 1. Added missing helper functions: is_super_admin(), get_user_partner_id()
-- 2. Fixed is_task_approver() to reference approver_user_id column (which now exists)
-- 3. Updated log_activity() function to properly handle all columns now in user_activity_log
-- 4. Fixed task approval/rejection functions to use correct column names
-- 5. All functions now reference columns that exist in corrected 009_phase4_tables.sql

-- ============================================================================
-- HELPER FUNCTIONS
-- ============================================================================

-- PHASE 4 FIX: Drop existing functions to avoid "cannot change name of input parameter" errors
DROP FUNCTION IF EXISTS public.is_super_admin(UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_user_partner_id(UUID) CASCADE;

-- Check if user is a super admin
-- PHASE 4 FIX: This function was referenced in policies but didn't exist
...
-- PHASE 4 FIX: Updated to use approver_user_id column (which now exists in corrected 009)
...
-- Log activity action
-- PHASE 4 FIX: Updated to use all columns now in user_activity_log table (from corrected 009)
-- Now properly handles: partner_id, entity_type, entity_id, before_state, after_state, change_summary, ip_address, user_agent
...
-- Approve task
-- PHASE 4 FIX: Now references partner_id and approver_user_id which now exist in corrected 009
