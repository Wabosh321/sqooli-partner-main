...
import { listCurricula, listSubjects } from '../../lib/supabaseClient';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function EditProgramDialog({ open, onOpenChange, program }: EditProgramDialogProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [cData, sData] = await Promise.all([listCurricula(), listSubjects()]);
        if (!mounted) return;
        setCurricula((cData || []).map((c: any) => ({ ...c, _id: c.id })));
        setExistingSubjects((sData || []).map((s: any) => ({ ...s, _id: s.id })));
      } catch (err) {
